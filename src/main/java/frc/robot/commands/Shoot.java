...
public class Shoot extends Command {
  ...
  public void execute() {
    ...
    DogLog.log(
        "Shoot/tof",
        2 * shootingSpeed(target, 5) * Math.sin(Math.toRadians(shooterAngleDeg)) / 9.81);

    DogLog.log("Shoot/target", target);
    for (int i = 1; i <= 5; i++) {
      DogLog.log(
          "Shoot/positionTargeting/" + i + "prec",
          new Pose3d(
    ...
    Pose3d gunOffset =
        MiscMath.RotatedPosAroundVertical(
            Constants.Shooter.offset, drivetrain.getPose().getRotation().getRadians());
  ...
  public double turnRate(Pose3d targetNoOffset) {
    return -1
        * MiscMath.clamp(
            targetAngle(targetNoOffset) - drivetrain.getPose().getRotation().getRadians(),
            -maxRotSpeed,
            maxRotSpeed);
  }
  ...
  public double targetAngle(Pose3d targetNoOffset) {
    ...
    return Math.atan2(
            Vector3.subtract(target, new Vector3(drivetrain.getPose())).y,
            Vector3.subtract(target, new Vector3(drivetrain.getPose())).x)
        + (shootsback ? 3.14 : 0);
  }
  ...
  protected Vector3 positionToTarget(Pose3d target, int precision) { // fixed shooting speed, also Z is altitude btw
    ...
    double timeOfFlight =
        2 * shootingSpeed(target, precision) * Math.sin(Math.toRadians(shooterAngleDeg)) / 9.81;
