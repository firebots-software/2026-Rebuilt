...
public class RobotContainer {
  ...
  private void configureBindings() {
    ...
    if (Constants.shooterOnRobot && Constants.hopperOnRobot) {
      joystick.rightBumper().onTrue(new WarmUpAndShoot(10f, () -> true, lebron, hopperSubsystem));
    }

    if (Constants.climberOnRobot) {
      joystick.povUp().onTrue(new L3Climb(climberSubsystem, drivetrain, redside));
      joystick.povRight().onTrue(new L2Climb(climberSubsystem, drivetrain, redside));
      joystick.povDown().onTrue(new L1Climb(climberSubsystem, drivetrain, redside));
    }
      ...
      joystick
          ...
          .whileTrue(
              new ArcAroundAndShoot(
                  drivetrain, lebron, intakeSubsystem, hopperSubsystem, speedFunction, redside));
    }
